...
then
	...
	if [ $merge_full_res_fused_image = true -a $start_from_downsample = false ];
	then 
	...
	# if compute isoptropic from full res then just downsample
	# otherwise downsample the grid and then merge
	if [ $compute_isotropic_from_full_res = true ];
	then

		echo ""
		echo "###################"
		echo "Downsample Full Res"
		echo "###################"
		echo ""

		# calculate fused image size
		export fused_image_size_bytes=`du -bc ${full_res_path}fused_oblique_${out_res}.tif | head -1 | sed -e 's/\s.*$//'`
		export fused_image_size_gb=$(echo "$fused_image_size_bytes/1024/1024/1024" | bc -l)
		export fused_image_size=`python -c "from math import ceil; print(ceil($fused_image_size_gb))"`
		...
		export qsub_output=`qsub -N $job_name_downsample -cwd -binding linear_per_task:1 -pe threads $((threads_per_job/2)) -l m_mem_free="$((memory_per_thread*2))"G -p ${priority} $downsample_script`
